...
def split_data(sentences_en, sentences_de, text_vec_layer_de):
...
def create_architecture(text_vec_layer_en, text_vec_layer_de, embed_size=128, vocab_size=3000, 
...
def save_model(model, name):
    model.save(os.path.join(os.getcwd(), f"models/english-to-german/{name}.keras"))
...
if __name__ == "__main__":
    ...
    X_train, X_valid, X_train_dec, X_valid_dec, Y_train, Y_valid = split_data(sentences_en, sentences_de, text_vec_layer_de)

    model = create_architecture(text_vec_layer_en, text_vec_layer_de)

    model = compile_model(model)

    history = train_model(model, X_train, X_train_dec, Y_train, X_valid, X_valid_dec, Y_valid, epochs=1)

    save_model(model, 'en-to-ge-model')